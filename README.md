...
1. `LuopleGit-Setup-0.5.4.exe`를 실행합니다.
...
0.5.4부터 `lu i` 안에서 충돌을 해결하고 저장할 수 있습니다. 진행 중인 통합이 있으면 바로 검토 화면을 엽니다. 터미널에서 `lu i connect`를 실행해도 미리보기 준비 후 검토 메뉴로 이어집니다.

1. 충돌 파일을 선택하고 `두 내용 비교하기`로 차이를 확인합니다.
2. `내 PC 파일 사용` 또는 `가져온 파일 사용`을 선택하고 확인합니다. 선택은 파일 전체에 적용되며, 선택한 쪽에 파일이 없으면 삭제됩니다.
3. 직접 합치려면 `직접 수정하기`로 편집기를 열고 저장한 뒤 `수정 완료 확인`을 선택합니다. Windows 텍스트 파일은 메모장, 바이너리는 검토 폴더를 엽니다.
4. 모든 충돌이 해결되면 `통합하고 저장하기`를 선택합니다. `finish --resolved`를 직접 입력하지 않아도 됩니다.

`나중에 하기`는 해결 내용을 보존합니다. 다시 `lu i`를 실행하면 이어서 검토합니다. 기존 버전에서 만든 통합 미리보기도 사용할 수 있습니다. 폴더·심볼릭 링크 같은 특수 충돌은 직접 검토해야 합니다.
